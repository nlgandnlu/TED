...
class LMDataset(torch.utils.data.Dataset):
    def __init__(self, block_size, data_path, label_path, index_path, tokenizer: PreTrainedTokenizer):

        # self.block_size=block_size
        # self.data_path=data_path
        # self.label_path=label_path
        # self.tokenizer=tokenizer
        self.labels = self._get_exmples(label_path, tokenizer, block_size)
        self.zhutis = self._get_exmples(data_path,tokenizer,block_size)
        self.indexs = self._get_indexs(index_path)
    ...
    def __getitem__(self, i) -> torch.Tensor:
        return [torch.tensor(self.zhutis[i], dtype=torch.long), torch.tensor(self.labels[i], dtype=torch.long), torch.tensor(self.indexs[i], dtype=torch.long)]


def get_dataset(args: DataTrainingArguments, tokenizer: PreTrainedTokenizer, evaluate=False, local_rank=-1):
    file_path = args.eval_data_file if evaluate else args.train_data_file
    concept_path = args.eval_concept_file if evaluate else args.train_concept_file
    index_path = args.eval_index_file if evaluate else args.train_index_file
    if args.line_by_line:
        # return LineByLineTextDataset(
        #     tokenizer=tokenizer, file_path=file_path, block_size=args.block_size
        # )
        return LMDataset(
            tokenizer=tokenizer, label_path=file_path, data_path=concept_path, index_path=index_path, block_size=args.block_size
        )
    else:
        return TextDataset(
            tokenizer=tokenizer, file_path=file_path, block_size=args.block_size
...
def main():
    ...
    special_tokens_dict = {'bos_token': '[SOS]', 'cls_token': '[CLS]', 'sep_token': '[SEP]'}
    ...
    data_collator = DataCollatorForLanguageModeling(
        tokenizer=tokenizer, mlm=data_args.mlm, mlm_probability=data_args.mlm_probability, contrast=training_args.contrast, contrast_ce=training_args.contrast_ce
    )
